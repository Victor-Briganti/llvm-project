...
kmp_int32 __kmpc_memo_in(ident_t *loc, kmp_int32 global_tid, kmp_int32 hashloc,
                         kmp_int32 radius, void *output, kmp_int32 type_out) {
  if (!__kmp_approx)
    return 1;

  __kmp_assert_valid_gtid(global_tid);
  return __kmp_memo_in(hashloc, global_tid, radius, output,
                       (memo_num_t)type_out);
}
...
void __kmpc_memo_out(ident_t *loc, kmp_int32 global_tid, kmp_int32 hashloc,
  ...
  __kmp_assert_valid_gtid(global_tid);
  __kmp_memo_out(hashloc, global_tid, radius, output, (memo_num_t)type_out);
}
